				...
				<div class="container">
					<div class="well">
						<div class="row">
							<div class="col-sm-10">
								<div class="input-group margin-bottom-sm">
									<?php if(isset($_GET['filtro'])){ ?>
										<span class="input-group-addon"><i class="fa fa-search fa-fw"></i></span> <input type="text" value='<?php echo $_GET['filtro'] ?>' class="input-search form-control" alt="tabela-dados" placeholder="Buscar número do processo, descrição, mês, ou responsável" id="search" autofocus="autofocus" />
									<?php }else{ ?>
							...
							<div class="col-sm-2 col-xs-12 pull-right">
									<a href="pdf-processos.php?sessionId=<?php echo $num ?>&tipo=todos" class="btn btn-sm btn-info pull-right"><i class="fa fa-file"></i> Gerar PDF</a>
							</div>
						</div>
					</div>
					<div class="col-md-12 table-responsive" style="overflow: auto; width: 100%; height: 300px;">
						<table class="table table-hover tabela-dados">
							...
							<tbody>
								<?php $lista = retorna_processos($conexao_com_banco);
								...
								<?php 
									...
									<tr style="background-color: #e74c3c; color:white;"> <?php } ?>
										<tr>
											...
											<td><?php echo $r-> CD_SETOR_LOCALIZACAO ?></td>	
											<td><?php echo $r-> NR_DIAS; ?></td>	
										    <!-- botão que quando apertado abre o modal -->
											<?php $permissao = retorna_permissao($_SESSION['CPF'],'FAZER_OPERACOES_OUTROS_ORGAO',$conexao_com_banco); if($permissao=='sim' or $r-> CD_SERVIDOR_LOCALIZACAO == $_SESSION['CPF']){ ?>											
											<td>
												<center>
														<a href='processo-detalhes.php?sessionId=<?php echo $num ?>&processo=<?php echo $r -> CD_PROCESSO ?>
														&pagina=-todos'><button id='detalhes' type='button' class='btn btn-default btn-sm' data-toggle='modal'
											...
											<td>
												<center>
													<?php $permissao = retorna_permissao($_SESSION['CPF'],'EDITAR_PROCESSO',$conexao_com_banco); $permissao2 = retorna_permissao($_SESSION['CPF'],'FAZER_OPERACOES_OUTROS_ORGAO',$conexao_com_banco); if($permissao=='sim' and $permissao2=='sim'){ ?>
														<a href="edita-processo.php?sessionId=<?php echo $num ?>&processo=<?php echo $r -> CD_PROCESSO ?>&pagina=-todos"><button type='button' class='btn btn-secondary btn-sm' title="Editar"><i class="fa fa-pencil" aria-hidden="true"></i></button></a>
													<?php } ?>
													<?php $permissao = retorna_permissao($_SESSION['CPF'],'EDITAR_PROCESSO',$conexao_com_banco); $permissao2 = retorna_permissao($_SESSION['CPF'],'FAZER_OPERACOES_OUTROS_ORGAO',$conexao_com_banco); if($permissao=='sim' and $permissao2=='sim'){ ?>
														<a href="../componentes/processo/logica/excluir.php?sessionId=<?php echo $num ?>&processo=<?php echo $r -> CD_PROCESSO ?>&pagina=-todos"><button type='button' class='btn btn-secondary btn-sm' title="Excluir"><i class="fa fa-trash" aria-hidden="true"></i></button></a>
													<?php } ?>
