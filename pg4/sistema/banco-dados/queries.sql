...
UPDATE tb_bem_patrimonial doc SET CD_SETOR_LOCALIZACAO = (SELECT ID from tb_setores proc WHERE proc.CD_SETOR=doc.CD_SETOR_LOCALIZACAO);
...
ALTER TABLE `tb_bem_patrimonial` CHANGE `ID` `ID` INT(6) NOT NULL AUTO_INCREMENT;

ALTER TABLE `tb_chamados` CHANGE `CD_SERVIDOR_REQUISITANTE` `CD_SERVIDOR_REQUISITANTE` INT(6) NOT NULL;
...
UPDATE `tb_setores` SET `CD_SETOR_SUBORDINADO` = '6' WHERE `tb_setores`.`ID` = 8;
UPDATE `tb_setores` SET `CD_SETOR_SUBORDINADO` = '7' WHERE `tb_setores`.`ID` = 9;
UPDATE `tb_setores` SET `CD_SETOR_SUBORDINADO` = '11' WHERE `tb_setores`.`ID` = 10;

ALTER TABLE `tb_setores` CHANGE `CD_SETOR_SUBORDINADO` `ID_SETOR_SUBORDINADO` INT(6) NOT NULL;
...
DELETE FROM `tb_bem_patrimonial` WHERE ID_SETOR_LOCALIZACAO NOT IN (SELECT ID FROM tb_setores);

ALTER TABLE `tb_bem_patrimonial` ADD FOREIGN KEY (`ID_SETOR_LOCALIZACAO`) REFERENCES `tb_setores`(`ID`) ON DELETE RESTRICT ON UPDATE RESTRICT;

ALTER TABLE `tb_bem_patrimonial` CHANGE `NM_ANOS` `NR_ANOS` INT(3) NOT NULL;

DELETE FROM `tb_chamados` WHERE ID_SERVIDOR_REQUISITANTE NOT IN (SELECT ID FROM tb_setores);
...
ALTER TABLE `tb_despesas` ADD FOREIGN KEY (`ID_DESPESA`) REFERENCES `tb_despesas`(`ID`) ON DELETE RESTRICT ON UPDATE RESTRICT;
...
ALTER TABLE `tb_bem_patrimonial` CHANGE `VLR_AQUISICAO` `VL_AQUISICAO` FLOAT NOT NULL, CHANGE `VLR_RESIDUAL` `VL_RESIDUAL` FLOAT NOT NULL, CHANGE `VLR_DEPRECIAVEL` `VL_DEPRECIAVEL` FLOAT NOT NULL, CHANGE `VLR_DEPRECIACAO_MES` `VL_DEPRECIACAO_MES` FLOAT NOT NULL, CHANGE `VLR_DEPRECIACAO_ACUMULADA` `VL_DEPRECIACAO_ACUMULADA` FLOAT NOT NULL, CHANGE `VLR_LIQUIDO` `VL_LIQUIDO` FLOAT NOT NULL;
