...
function listar_atividades() {
	...
	success: function(data) {
		...
		for (var i = 0; i < data.length; i++) { 
			...
			if (!data[i].DATA_SAIDA) {
				mapa_exibicao[parseInt(new Date(data[i].DATA_ENTRADA).getMonth())]['Aberta'] ++; 
			} else if (data[i].Dias <= 3) {
				mapa_exibicao[parseInt(new Date(data[i].DATA_ENTRADA).getMonth())]['Realizadas_Prazo'] ++; 
			} else {
				mapa_exibicao[parseInt(new Date(data[i].DATA_ENTRADA).getMonth())]['Incompletas'] ++; 
			}			
			console.log(parseInt(new Date(data[i].DATA_ENTRADA).getMonth()));
			mapa_exibicao[parseInt(new Date(data[i].DATA_ENTRADA).getMonth())]['Total'] ++; 
			
		}
		$("#atividades-geral").empty();
		for (i = 0; i < 12; i++) {
			$("#atividades-geral").append('<tr><td>' + nomes_mes[i] + '</td><td style="text-align: center;">' + mapa_exibicao[i]['Aberta'] + '</td><td style="text-align: center;">' + mapa_exibicao[i]['Realizadas_Prazo'] + '</td><td style="text-align: center;">' + mapa_exibicao[i]['Realizadas_Vencidas'] + '<td style="text-align: center;">' + mapa_exibicao[i]['Total'] + '</td>' +
	...
	success: function(data) {
		...
		for (var i = 0; i < data.length; i++) { 
			...
			if (data[i].NM_STATUS.indexOf('CONCLUÍDO') === -1) {
				texto += '<a href="javascript:void(0);" onclick="remover_atividade(' + data[i].CD_ATIVIDADE + ')">' +
						...
						<table class="table table-hover tabela-dados">
							<thead>
								<tr>
									...
									<th style="text-align: center;">Tramitados (prazo)</th>
									<th style="text-align: center;">Tramitados (atraso)</th>														
