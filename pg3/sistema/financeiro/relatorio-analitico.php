				...
				$lista = retorna_despesas_tipo('Fixa', $ano, $conexao_com_banco); 
					while($r = mysqli_fetch_object($lista)) { $codigo = $r->CD_DESPESA;
						...
						while($r2 = mysqli_fetch_object($lista2)){ 
							$html .='<tr>
							<td style="text-align: left; padding-left: 0px; width: 151px;">'.
							$r2->DS_DESPESA.'</td>';
							
							 for($i=1;$i<13;$i++){
								$html .='<td style="text-align: center; color: red;">'.arruma_numero(retorna_valor_despesa_paga_descricao($r2->CD_DESPESA,$r2->DS_DESPESA,$i,$ano,$conexao_com_banco)).'</td>';
							 }
							$html .='</tr>';
						}	
