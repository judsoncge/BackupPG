...
function retorna_processos_prazo_final_hoje($conexao_com_banco){
	
	$data = date("Y-m-d");
	
	$resultado = mysqli_query($conexao_com_banco, "SELECT count(*) FROM tb_processos WHERE DT_PRAZO_FINAL='$data' and NM_STATUS='Ativo'");
...
function retorna_processos_prazo_parcial_hoje($conexao_com_banco){
...
function retorna_servidores_relatorio_pessoa($conexao_com_banco){
	...
	if($_SESSION['permissao-visualizar-relatorio-setor']=="sim"){
		$resultado = mysqli_query($conexao_com_banco, "SELECT * FROM tb_servidores WHERE CD_SETOR='$setor' or CD_SETOR='$subordinado'");
	}elseif($_SESSION['permissao-visualizar-relatorio-orgao']=="sim"){
		$resultado =  mysqli_query($conexao_com_banco, "SELECT * FROM tb_servidores");
	}
...
function retorna_processos_com_servidor($CPF, $conexao_com_banco){
	...
	$resultado = mysqli_query($conexao_com_banco, "SELECT p.*, s.NM_SERVIDOR AS NM_SERVIDOR_LOCALIZACAO FROM tb_processos AS p LEFT JOIN tb_servidores s on p.CD_SERVIDOR_LOCALIZACAO = s.CD_SERVIDOR WHERE CD_SERVIDOR_LOCALIZACAO='$CPF' and NM_STATUS='Ativo' ORDER BY DT_PRAZO");
...
function retorna_processos_status($status, $conexao_com_banco){
	...
	$resultado = mysqli_query($conexao_com_banco, "SELECT p.*, s.NM_SERVIDOR AS NM_SERVIDOR_LOCALIZACAO FROM tb_processos AS p LEFT JOIN tb_servidores s on p.CD_SERVIDOR_LOCALIZACAO = s.CD_SERVIDOR WHERE NM_STATUS='$status'");
...
function retorna_podem_ser_responsaveis_processo($processo, $conexao_com_banco){
	...
	FROM tb_servidores, permissao WHERE tb_servidores.CD_SERVIDOR = permissao.CD_SERVIDOR and permissao.SER_RESPONSAVEL_PROCESSO='sim' and tb_servidores.CD_SERVIDOR not in(SELECT CD_SERVIDOR FROM tb_responsaveis_processos WHERE CD_PROCESSO='$processo') order by tb_servidores.NM_SERVIDOR";
...
function retorna_numero_processos_setor_situacao($setor, $situacao, $conexao_com_banco){
	...
	$resultado = mysqli_query($conexao_com_banco, "SELECT COUNT(*) FROM tb_processos WHERE (CD_SETOR_LOCALIZACAO='$setor' 
	or CD_SETOR_LOCALIZACAO='$subordinado') and NM_SITUACAO='$situacao' and NM_STATUS='Ativo'");
