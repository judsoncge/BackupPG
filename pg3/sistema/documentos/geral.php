...
var pagina = 0, max = 100, status = '', lugar = '';
...
function listar_documentos(nova_pagina) {
	...
	success: function(data, status, resposta) {
		...
		toAppend += '<td><a href="../processos/detalhes.php?processo=' + data[i].CD_PROCESSO + '">' + data[i].CD_PROCESSO + '</a></td>' +
		...
		'<td><center>';
		if (data[i].EDITAR) {
			toAppend += '<a href="editar.php?documento=' + data[i].CD_DOCUMENTO + '">' +
		...
		if (data[i].EXCLUIR) {
			toAppend += '<a href="logica/excluir.php?documento=' + data[i].CD_DOCUMENTO + '" onclick="return confirm(\'Você tem certeza que deseja apagar este documento?\');">' +
						...
						<div class="row">
							<div class="col-sm-4 filter-opt">
								...
								<div class="btn-group search lugar" role="group">
									  <a href="javascript:void(0);" onclick = "listar_lugar('todos');")><button type="button" id="listar_lugar_todos" class="btn ativo botao-comunicacao lugar">Todas</button></a>
									  ...
									  <a href="javascript:void(0);" onclick = "listar_lugar('comigo');"><button type="button" id="listar_lugar_comigo" class="btn botao-comunicacao lugar">Comigo</button></a>									 							  
								</div>
							</div>
