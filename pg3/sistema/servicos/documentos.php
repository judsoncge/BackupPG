...
function listar_documentos($busca_query, $lugar, $status, $max, $offset,$conexao_com_banco) {
	...
	if (!empty($busca_query)) {
		$busca_query = " WHERE (d.CD_PROCESSO LIKE '%$busca_query%' OR d.NM_DOCUMENTO LIKE '%$busca_query%' OR d.NM_ATIVIDADE LIKE '%$busca_query%' OR d.NM_STATUS LIKE '%$busca_query%' OR s.CD_SERVIDOR LIKE '%$busca_query%' OR s.NM_SERVIDOR LIKE '%$busca_query%' OR d.DS_DOCUMENTO LIKE '%$busca_query%' OR d.CD_SETOR_LOCALIZACAO LIKE '%$busca_query%')";
	}
	...
	$query = "Select d.*, s.NM_SERVIDOR as NM_SERVIDOR_LOCALIZACAO, s2.NM_SERVIDOR as NM_SERVIDOR_CRIACAO FROM `tb_documentos` d LEFT JOIN `tb_servidores` s ON d.CD_SERVIDOR_LOCALIZACAO = s.CD_SERVIDOR LEFT JOIN `tb_servidores` s2 ON d.CD_SERVIDOR_CRIACAO = s2.CD_SERVIDOR $busca_query LIMIT $max OFFSET $offset";
	//echo $query;
	...
	$query_total = "Select count(*) as total_documentos FROM `tb_documentos` d LEFT JOIN `tb_servidores` s ON d.CD_SERVIDOR_LOCALIZACAO = s.CD_SERVIDOR $busca_query";
	//echo $query_total;
