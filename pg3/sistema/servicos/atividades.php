...
function listar_atividades_gabinete_andamento($conexao_com_banco, $cd_servidor) {
	$atividades = mysqli_query($conexao_com_banco, "Select t.CD_PROCESSO as Processo, DATEDIFF(CURDATE(),t.DT_TRAMITACAO) as Dias, t.DT_TRAMITACAO Data_Entrada FROM `tb_tramitacao_processos` t, `tb_processos` p WHERE t.CD_SERVIDOR_DESTINO = '$cd_servidor' AND p.CD_PROCESSO = t.CD_PROCESSO and p.NM_STATUS NOT IN ('Arquivado', 'Saiu') AND t.CD_PROCESSO NOT IN (SELECT CD_PROCESSO FROM `tb_tramitacao_processos` where CD_PROCESSO = t.CD_PROCESSO AND DT_TRAMITACAO > t.DT_TRAMITACAO and CD_SERVIDOR_ORIGEM = t.CD_SERVIDOR_DESTINO)") or die (mysqli_error($conexao_com_banco));
